...
- **CSS Utilities**: `.premium-card` (gradient card bg, light+dark), `.skeleton-shimmer` (gold-accent shimmer via `--hndld-gold-400`), `.hndld-glass` (backdrop-blur glassmorphism), `.grain-overlay` (SVG noise texture at 2.5% opacity)
