...
- **Predictive Home Maintenance Intelligence** (server/services/predictive-maintenance.ts): Appliance lifecycle analysis using industry lifespan data + network analytics (k-anonymity safe). Consumable tracking with replacement reminders. Warranty expiration actions. AI-powered predictive insights via generateAIPredictiveInsight. Integrated into home-intelligence.ts (generateAllInsights) and ai-agent.ts (proactive maintenance insights). Schema: applianceConsumables (knowledge base, seeded with 14 common records), householdConsumableTracking (per-item tracking with intervals/due dates). API routes in server/routes/inventory.ts: GET /inventory/predictions (all predictions), GET /inventory/:id/health (item health assessment with risk level/lifespan %), GET /inventory/:id/consumables (matched consumables for item), POST /inventory/:id/consumables/track, PATCH /inventory/consumables/tracking/:id/replace. Frontend: "Home Health" prediction alerts section on inventory Items tab, Health Assessment card in item detail, Consumables & Supplies section with track/replace/buy actions. Insight categories: APPLIANCE_LIFESPAN, CONSUMABLE_REMINDER, WARRANTY_ACTION, REPLACEMENT_FORECAST
...
- **Document & Contract Tracking** (server/routes/documents.ts): Track insurance policies, warranties, contracts, licenses with expiration alerts. CRUD endpoints: GET/POST/PATCH/DELETE /documents, GET /documents/expiring?days=N, GET /documents/summary (stats by type, annual cost, expiring/expired counts), GET /documents/:id. Supports document types (10 insurance/warranty/contract/license types), financial fields (annual cost, coverage, deductible in cents), date tracking (effective, expiration, renewal), alert settings (days before), auto-renewal flag, provider contact info. Frontend at `/documents` with summary cards, search, type filter pills, grouped list with expiry countdown badges, detail view with contact links, add/edit form. Table: tracked_documents
- **Budget Tracking** (server/routes/budgets.ts): Track budgets against actual spending per category. CRUD endpoints: GET/POST/PATCH/DELETE /budgets, GET /budgets/status (all budgets with actual vs budget calculations), GET /budgets/:category/history (6-month historical spending vs budget), GET /budgets/alerts, PATCH /budgets/alerts/:id/acknowledge. Supports monthly/quarterly/annual periods, configurable alert threshold (default 80%), color-coded progress bars (green <70%, amber 70-90%, red >90%). Background scheduler (daily 8am) checks thresholds and sends notifications. Frontend at `/budgets` with summary cards, progress bars, quick edit, history charts. Tables: budgets, budget_alerts
- **Guest Access** (server/routes/guest-access.ts): Temporary access for guests, house sitters, contractors. POST /guest-access/invite (creates invitation with token), GET /guest-access (list all), GET /guest-access/accept/:token (accept invite), POST /guest-access/:id/revoke (revoke with reason), PATCH /guest-access/:id, DELETE /guest-access/:id. Four access levels (VIEW_ONLY, LIMITED, STANDARD, FULL) with granular permissions (canViewTasks, canViewCalendar, canViewVendors, canViewFiles, canSendMessages, canCreateTasks). Time-bounded with start/end dates. Statuses: PENDING, ACTIVE, EXPIRED, REVOKED. Background scheduler (every 30 minutes) auto-expires access and notifies household. Frontend at `/guest-access` with active/pending/past sections, invite form with date range picker, access level presets, custom permissions, revoke dialog. Table: guest_access
- **In-App Feedback** (server/routes/feedback.ts): User feedback submission with screenshot upload (sharp resize + webp), type classification (BUG, FEATURE_REQUEST, GENERAL, COMPLAINT, PRAISE), status tracking (NEW, REVIEWED, IN_PROGRESS, RESOLVED, WONT_FIX). User endpoints: POST /feedback (multipart with optional screenshot), GET /feedback (user's submissions), GET /feedback/:id (detail + replies), POST /feedback/:id/reply. Admin endpoints: GET /feedback/admin/all (filterable by status), PATCH /feedback/:id/status (update status/adminNotes), POST /feedback/:id/admin-reply (reply as support staff). Floating FeedbackButton in app layout, FeedbackDialog with type selector, My Feedback page at `/my-feedback` with detail/reply view. Tables: feedback, feedback_replies
- **Database Tables**: subscriptions, payment_methods, invoices, analytics_events, emergency_contacts, emergency_protocols, conversations, messages, ai_settings, weekly_briefs, user_engagement, household_connections, vendor_reviews, vendor_shares, referrals, group_buy_requests, group_buy_offers, group_buy_participants, backup_providers, emergency_coverage_requests, celebrations, handwritten_notes, feedback, feedback_replies, tracked_documents
- **Receipt Scanning** (server/routes/receipts.ts, server/services/receipt-ocr.ts): AI-powered receipt scanning using Anthropic Claude vision. POST /receipts/scan uploads image and extracts vendor/date/total/category/items/tax. POST /receipts/confirm saves extracted data as spending item with receipt image stored as file attachment. Client component (client/src/components/spending/receipt-scanner.tsx) integrated into Assistant spending page via "Scan" button. Three-step flow: upload (camera/file) → scanning (AI extraction with preview) → review (editable fields + confidence indicator + detected items). Falls back to demo data when no API key available.
- **Annual Financial Reports** (server/routes/reports.ts): Tax preparation reports. GET /reports/annual/:year returns JSON with spending by category, by vendor, monthly trend, potentially deductible expenses (Maintenance/Utilities/Services/Insurance/Home Office/Home Improvement), and vendors paid $600+ (1099 reporting). GET /reports/annual/:year/pdf generates formatted PDF report via pdfkit. GET /reports/annual/:year/csv exports all spending items as CSV. Frontend at `/reports` with year selector (back 5 years), summary cards (total spent, transactions, deductible total, 1099 vendor count), monthly bar chart, category breakdown with deductible badges and progress bars, 1099 vendor alert section, top vendors list, PDF/CSV download buttons. Includes tax disclaimer.

- **Multi-Property Support** (server/routes/properties.ts): Manage multiple properties per household. Schema: properties table (name/type/address/city/state/postalCode/timezone/bedrooms/bathrooms/squareFootage/yearBuilt/photoUrls/isPrimary/settings/accessInstructions/alarmCode+wifiPassword encrypted via vault). Added propertyId column to: tasks, vendors, calendarEvents, cleaningVisits, smartLocks, trackedDocuments, budgets. Property types: PRIMARY, VACATION, RENTAL, INVESTMENT, FAMILY, OTHER. Endpoints: GET/POST/PATCH/DELETE /properties, GET /properties/:id (with ?sensitive=true to decrypt), POST /properties/:id/set-primary. Soft delete (isActive=false). Frontend at `/properties` with property cards (type icon, location, task/vendor/lock counts), add property dialog (full form with name/type/address/details/access info), property detail view with location/details/access information cards, reveal/hide toggle for alarm code and WiFi password, set-as-primary button. Property switcher component (client/src/components/property-switcher.tsx) provides context + dropdown for filtering across pages.
- **OAuth2 Server** (server/routes/oauth.ts): Account linking for Alexa and Google Assistant. Schema: oauthClients (clientId/clientSecret hashed via bcrypt/name/redirectUris/scopes/isActive), oauthAuthorizationCodes (code/clientId/userId/householdId/redirectUri/scope/expiresAt/usedAt). GET /oauth/authorize renders consent page (porcelain+navy themed) with permission list. POST /oauth/authorize generates authorization code (10-min expiry), redirects with code+state. POST /oauth/token exchanges code for long-lived access token (365 days) stored in apiTokens table. Validates client_secret via bcrypt, enforces redirect_uri match, marks codes as used. Registered on v1 router.
- **Google Assistant Action** (server/routes/google-assistant.ts, actions.yaml): Voice commands via Google Assistant using @assistant/conversation SDK. POST /google-assistant webhook. Handlers: main_invocation (welcome + suggestions), next_cleaning, pending_approvals, approve_all, spending_summary, create_task (with task_title param), lock_door/unlock_door (with door_name param), fallback. Uses apiTokens for account linking via OAuth2. Actions config in actions.yaml. Registered outside v1 router for direct Google access.
- **Amazon Alexa Skill** (server/routes/alexa.ts, alexa-skill.json): Voice commands via Alexa. POST /alexa endpoint handles all Alexa requests. Intents: NextCleaningIntent (next cleaning visit), PendingApprovalsIntent (count pending), ApproveAllIntent (bulk approve), SpendingSummaryIntent (spending by period with AMAZON.DATE slot), CreateTaskIntent (create tasks via voice), LockDoorIntent/UnlockDoorIntent (smart lock control via AMAZON.Room slot), GuestAccessIntent (active guest access list). Uses apiTokens for account linking. Registered outside v1 router for direct Alexa access. Skill config in alexa-skill.json with invocation name "hndld".
- **B2B Service Provider Portal** (server/routes/provider/index.ts): Parallel B2B platform for service providers (cleaning companies, PAs, handyman, landscaper, pool, pet care, meal prep, organizing). Schema: serviceProviders (businessName/type/email/contact/address/licensing/insurance/subscription/verification/stats), providerStaff (name/email/phone/role/hourlyRate/isActive), providerClients (provider↔household link with serviceFrequency/preferredDay/rates/accessInstructions), providerSchedule (date/time/duration/status lifecycle SCHEDULED→CONFIRMED→IN_PROGRESS→COMPLETED/CANCELLED/NO_SHOW). Provider auth via ownerId linking. Endpoints: POST /provider/register, GET/PATCH /provider/me, GET /provider/dashboard (client/staff/schedule stats with upcoming), GET/POST/PATCH /provider/clients (CRUD with household linking), GET/POST/PATCH/DELETE /provider/staff, GET/POST/PATCH/DELETE /provider/schedule (date/staff/status filtering), GET /provider/invoices (completed visit revenue), GET /provider/leads (marketplace placeholder). Frontend at `/provider` with tabbed portal: Dashboard (stat cards, upcoming schedule), Clients (list/add/detail with access instructions), Staff (add/activate/deactivate with roles), Schedule (upcoming visits with status progression buttons), Invoices (completed visit revenue tracking), Settings (business info, location, insurance/licensing). Registration flow for new providers with 14-day trial.
- **IFTTT-Style Automations** (server/routes/automations.ts, server/services/automation-engine.ts): Automation system for household routines. Schema: automations (trigger/triggerConfig/conditions/actions/isEnabled/isPaused/pauseUntil/runCount/lastRunAt/lastRunStatus), automationRuns (triggeredBy/status/actionsExecuted/error/startedAt/completedAt). 20 trigger types: SMART_LOCK_UNLOCK/LOCK, APPROVAL_CREATED/PENDING_HOURS/APPROVED/REJECTED, BUDGET_THRESHOLD/EXCEEDED, TASK_CREATED/COMPLETED/OVERDUE, CLEANING_STARTED/COMPLETED, SCHEDULE_TIME/DAY, DOCUMENT_EXPIRING, SPENDING_CREATED, CALENDAR_EVENT_SOON, GUEST_ACCESS_STARTED/ENDED. 13 action types: SEND_NOTIFICATION/EMAIL/SMS, CREATE_TASK/COMPLETE_TASK, CREATE_APPROVAL/AUTO_APPROVE, LOCK_DOOR/UNLOCK_DOOR, ADD_TO_CALENDAR, UPDATE_BUDGET, TRIGGER_WEBHOOK, LOG_EVENT. Engine: processTrigger() finds matching automations by householdId+trigger+isEnabled, checks conditions (userIds/vendorIds/taskCategories/minAmount/maxAmount), executes ordered action chain, logs runs. Condition matching supports user/vendor/category/amount filters. Template interpolation with {{variable}} syntax. 8 pre-built templates: lock-after-cleaning, approval-reminder, budget-alert, task-overdue-escalate, guest-arrival, document-expiry, spending-webhook, daily-morning-routine. Endpoints: GET/POST/PUT/DELETE /automations, GET /automations/templates, GET /automations/:id/runs, POST /automations/:id/test, POST /automations/:id/pause, POST /automations/:id/resume. Frontend at `/automations` with automation list (enable/disable toggle, test/pause/edit/history), step-by-step builder (template selector, trigger picker by category, action chain builder with inline config, finalize with name/description/color), run history view with status timeline, templates gallery.
- **Smart Lock Integration** (server/routes/smart-locks.ts, server/services/smart-locks.ts): Manage smart locks and access codes for household doors. Schema: smartLocks (provider/name/externalId/accessToken/refreshToken/isConnected), lockAccessCodes (code/name/vendorId/guestAccessId/personId/schedule/expiry), lockAccessLog (action/method/timestamp/metadata). Providers: AUGUST, SCHLAGE, YALE, LEVEL, OTHER with pluggable provider abstraction. Endpoints: GET/POST/PATCH/DELETE /smart-locks, POST /smart-locks/:id/lock, POST /smart-locks/:id/unlock, GET/POST /smart-locks/:id/codes, PATCH/DELETE /smart-locks/:lockId/codes/:codeId, GET /smart-locks/:id/activity. Access code scheduling: ALWAYS, TEMPORARY (with expiry), SCHEDULED (days of week + time window). Codes link to vendors, guests, or household people. All actions logged to lockAccessLog. Frontend at `/smart-locks` with lock list, lock/unlock controls, battery status, tabbed codes/activity view, add lock dialog (name/provider/device ID), add code dialog (name/code/schedule type/days/times/expiry), code toggle on/off, activity timeline.
